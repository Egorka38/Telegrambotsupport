...
MODERATOR_CHAT_ID = int(os.getenv('SUPPORT_CHAT_ID'))  # Замените на ID вашего чата поддержки
SUPPORT_CHAT_ID = int(os.getenv('SUPPORT_CHAT_ID'))  # Замените на ID вашего чата поддержки
if TOKEN is None:
...
# Добавление словаря для хранения соответствий между сообщениями в канале и пользователями
message_map = {}  # Словарь для хранения соответствий между сообщениями в канале и пользователями
user_states = {}  # user_id -> 'ожидание вопроса' / None
...
def handle_user_message(message):
    msg = bot.send_message(SUPPORT_CHAT_ID,f"📥 Новая заявка от @{message.from_user.username} (ID: {message.from_user.id}):\n\n{message.text}")
    # Привязываем ID модераторского сообщения к user_id
    message_map[msg.message_id] = message.from_user.id

    # отвечаем пользователю
    bot.send_message(user_id, "✅ Ваша заявка отправлена. Ожидайте ответа.") # type: ignore

    keyboard = types.InlineKeyboardMarkup()
    # добавляешь свои кнопки сюда если нужно

    # Пересылаем заявку в чат модераторов и сохраняем связь
    forwarded = bot.send_message(MODERATOR_CHAT_ID,f"📥 Новая заявка от @{message.from_user.username} (ID: {message.from_user.id}):\n\n{message.text}")
    message_links[forwarded.message_id] = message.chat.id  # сохранили ID чата пользователя

# Обработчик сообщений в чате поддержки для пересылки ответов пользователю
@bot.message_handler(func=lambda message: message.chat.id == SUPPORT_CHAT_ID)
def handle_support_reply(message):
    # Ищем последнего ID сообщения, на который модератор ответил
    ...
    if replied_message:
        ...
        if user_id:
            try:
                bot.send_message(user_id, f"📩 Ответ модератора:\n\n{message.text}")
                print(f"✅ Ответ отправлен пользователю {user_id}")
            except Exception as e:
                print(f"❌ Ошибка при отправке пользователю: {e}")
        else:
            bot.send_message(SUPPORT_CHAT_ID, "❗ Не найден пользователь для этого ответа.")
    else:
        bot.send_message(SUPPORT_CHAT_ID, "⚠️ Ответ должен быть через функцию 'Ответить'.")



@bot.callback_query_handler(func=lambda call: call.data == 'связь_с_менеджерами_menu')
def contact_managers(call):
    user_id = call.from_user.id
    bot.send_message(user_id, "Пожалуйста, напишите ваш вопрос менеджерам:")
    user_states[user_id] = {'state': 'manager', 'message_id': call.message.id}
    bot.answer_callback_query(call.id)

@bot.callback_query_handler(func=lambda call: call.data == 'custom_question')
def custom_question(call):
    user_id = call.from_user.id
    bot.send_message(user_id, "Пожалуйста, напишите ваш вопрос:")
    user_states[user_id] = {'state': 'question', 'message_id': call.message.id}
    bot.answer_callback_query(call.id)
